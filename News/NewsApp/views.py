...
def stock_price():
    ...
    #     stock['name']=v[0].text
    #     price=val[0].find_all('span',{'class':'Trsdu(0.3s) Fw(500) Fz(14px) C($negativeColor)'})
    #     print(price[0].text)
...
class HomeView(View):
    def get(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            ...
            print(text)
            url2 = f"https://newsapi.org/v2/everything?q={text}&apiKey=NEWS_API_KEY&sort-by=popularity"
        else:
            url2 = f"https://newsapi.org/v2/everything?q=corona&apiKey={NEWS_API_KEY}&sort-by=popularity"
        ...
        data = {
            "ChangeConfirm": val3[1]["Confirmed"] - val3[0]["Confirmed"],
            "ChangeRecover": val3[1]["Recovered"] - val3[0]["Recovered"],
            "ChangeDeath": val3[1]["Deaths"] - val3[0]["Deaths"],
        }
        content = {
            "TopHeadlines": val["articles"][0:9],
            "Popularity": val2["articles"][0:10],
            "Cdata": val3[1],
            "Change": data,
            "stock": stock_price(),
        }
    ...
    def post(self, request, *args, **kwargs):
        ...
        url3 = f"https://newsapi.org/v2/everything?q={news_topic}&apiKey={NEWS_API_KEY}"
...
def register(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            form.save()
            pref = Choices.objects.create(
                preferences=request.POST["topics2"],
